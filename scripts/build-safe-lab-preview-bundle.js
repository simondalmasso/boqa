...
function writeBundleChecksums(outputDir) {
  ...
  fs.writeFileSync(path.join(outputDir, 'SHA256SUMS'), lines, { flag: 'wx' });
  return files;
}
...
function buildSafeLabPreviewBundle(options) {
  ...
  const index = compileDashboardIndex(fs.readFileSync(indexPath, 'utf8'), build);
  ...
  const bundleFiles = writeBundleChecksums(outputDir);
  return { outputDir, policy, build, bundleFiles };
}
...
module.exports = { SAFE_LAB_WRANGLER, buildSafeLabPreviewBundle, compileDashboardIndex, readContract, replaceBuildBlock, verifyBuildObject, writeBundleChecksums };
